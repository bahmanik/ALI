...
const { title_map: userDefinedTitles } = options;
...
export const getWindowMatch = (hyprlandClient: AstalHyprland.Client): Record<string, string> => {
  ...
  // WARNING:userDefinedTitles should have get() when it changes to dynamic options
  const potentialWindowTitles = [...userDefinedTitles, ...defaultWindowTitleMap];
