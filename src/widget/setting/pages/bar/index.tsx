...
const Bar = (props: BarProps) => {
  ...
  return (
    <box orientation={Gtk.Orientation.VERTICAL} {...props}>
      ...
      <Option title="Margin top" subtitle="Gap above the bar (px)" opt={style.marginTop} type="number" min={0} max={60} increment={1} />
      <Option title="Margin bottom" subtitle="Gap below the bar (px)" opt={style.marginBottom} type="number" min={0} max={60} increment={1} />
      ...
      <Option title="Shadow blur" subtitle="Shadow blur radius (px)" opt={style.shadowBlur} type="number" min={0} max={80} increment={1} />
      <Option title="Shadow spread" subtitle="Shadow spread radius (px)" opt={style.shadowSpread} type="number" min={0} max={40} increment={1} />

      {/* ── Buttons ─────────────────────────────────────────────────────── */}
      <Header title="Buttons" />
      <Option title="Button BG" subtitle="Button background colour" opt={buttons.bg} type="color" />
      <Option title="Button BG opacity" subtitle="Button background opacity (0 – 100)" opt={buttons.bgOpacity} type="number" min={0} max={100} increment={1} />
      ...
      <Option title="Button radius" subtitle="Button corner radius (px)" opt={buttons.radius} type="number" min={0} max={40} increment={1} />
      <Option title="Button spacing" subtitle="Gap between buttons (px)" opt={buttons.spacing} type="number" min={0} max={40} increment={1} />
      <Option title="Button padding X" subtitle="Button horizontal padding (px)" opt={buttons.paddingX} type="number" min={0} max={40} increment={1} />
      <Option title="Button padding Y" subtitle="Button vertical padding (px)" opt={buttons.paddingY} type="number" min={0} max={40} increment={1} />
      ...
      <Option title="Icon" subtitle="Nerd Font glyph (paste the char)" opt={modules.cpu.icon} type="string" />
      <Option title="Round value" subtitle="Round the percentage to integer" opt={modules.cpu.round} type="boolean" />

      {/* CPU ring visual — the reusable CircularProgressSetting component */}
      <CircularProgressSetting label="CPU ring appearance" options={modules.cpu} />
    </box>
  )
}
