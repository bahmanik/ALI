...
class DesktopScanner {
  private entries = createState<DesktopEntry[]>([])
  ...
  private checkFlatpak(): void {
    try {
      ...
      if (this.hasFlatpak) {
        console.debug("Flatpak detected, will scan flatpak applications");
      } else {
        console.debug("Flatpak not found, skipping flatpak directories");
      }
    } catch (error) {
      this.hasFlatpak = false;
      console.debug("Flatpak check failed, assuming not installed");
    }
  }

  get applications() {
    return this.entries[0].peek();
  }
  ...
  private parseDesktopFile(path: string): DesktopEntry | null {
    try {
      ...
      console.debug(`Failed to parse desktop file ${path}: ${error}`);
      return null;
    }
  }
  ...
  private scanDesktopFiles(): DesktopEntry[] {
    ...
    for (const dirPath of desktopDirs) {
      try {
        ...
        console.debug(`Failed to scan directory ${dirPath}: ${error}`);
      }
    }
  ...
  private detectAppImages(): DesktopEntry[] {
    ...
    try {
      ...
      console.debug(`Failed to scan AppImage directory: ${error}`);
    }
  ...
  private createDesktopFileForAppImage(entry: DesktopEntry): boolean {
    try {
      ...
      console.info(`Created desktop file for AppImage: ${desktopFilePath}`);
      return true;
    } catch (error) {
      console.error(
  ...
  scan(): void {
    console.debug("Scanning for desktop files and AppImages...");
  ...
  createDesktopFilesForAllAppImages(): void {
    const appImages = this.entries[0]().filter((e) => e.isAppImage);
    let created = 0;
    ...
    if (created > 0) {
      console.info(`Created ${created} desktop files for AppImages`);
  ...
  fuzzySearch(query: string): DesktopEntry[] {
    ...
    const entries = this.entries[0].peek()
