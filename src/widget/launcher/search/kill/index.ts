...
async function getPortProcesses(): Promise<KillAction[]> {
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error("All port scanning methods failed", { ssError, netstatError, lsofError });
          return [];
        }
      }
    }
    ...
    console.error("Failed to get port processes", { error: err });
    return [];
  }
}
...
async function getAllRunningProcesses(): Promise<KillAction[]> {
  try {
    ...
    console.error("Failed to get running processes", { error: err });
    return [];
  }
}

export default async function getProcessResults(searchText: string, isPrefixSearch: boolean = false): Promise<KillButtonResult[]> {
  const query = searchText.toLowerCase().trim();

  console.debug("Getting kill results", { query, isPrefixSearch, cacheValid: isCacheValid() });
  ...
  if (!isCacheValid()) {
    console.debug("Cache expired or missing, fetching fresh data...");
...
export function clearKillCache() {
  cache = null;
  console.debug("Kill cache cleared");
}
