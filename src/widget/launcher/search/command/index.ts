...
export default function getCommandResults(searchText: string, isPrefixSearch: boolean = false): CommandButtonResult[] {
  ...
  console.debug("Getting command results", { query, isPrefixSearch });

  // If empty query with prefix, show history
  if (isPrefixSearch && query === '') {
    const history = commandHistory.peek();
    console.debug("Returning command history", { historyCount: history.length });
...
export function addToCommandHistory(command: string) {
  ...
  console.debug("Added to command history", { command, historySize: newHistory.length });
}
