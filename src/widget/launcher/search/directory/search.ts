...
class DirectorySearchService {
  ...
  constructor() {
    console.info("DirectorySearchService initialized");
  }
  ...
  async search(query: string, options: SearchOptions = {}): Promise<FileInfo[]> {
    ...
    if (query === "test") {
      console.info("Directory search TEST MODE activated");
      return [
    ...
    try {
      ...
      if (searchPaths.length === 0) {
        console.warn("No search paths found");
        return [];
      }

      console.debug("Search paths", { paths: searchPaths });

      let results: FileInfo[] = [];

      if (hasFd) {
        console.debug("Using fd for directory search");
        results = await this.searchWithFd(query, searchPaths, options);
      } else {
        console.debug("Using find for directory search (fd not found)");
        results = await this.searchWithSimpleFind(query, searchPaths, options);
      }

      console.debug("Raw search results", { count: results.length });
      ...
      console.debug("Directory search completed", {
        query,
  ...
  private async searchWithSimpleFind(query: string, paths: string[], options: SearchOptions): Promise<FileInfo[]> {
    ...
    for (const searchPath of paths) {
      try {
        ...
        console.debug("Running find command", { path: searchPath });
        ...
        console.debug("find failed or timed out for path", { path: searchPath, error });
  ...
  private async searchWithFd(query: string, paths: string[], options: SearchOptions): Promise<FileInfo[]> {
    ...
    try {
      ...
      console.debug("Running fd command", { cmd: cmd.join(" ") });
      ...
      console.debug("fd search completed", { resultCount: results.length });
      return results;

    } catch (error) {
      console.error("fd search failed", { error });
      return [];
    }
  }
