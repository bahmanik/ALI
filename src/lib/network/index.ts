...
class Network {
  ...
  private readonly _networkCheck = "http://www.google.com/generate_204";
  constructor(defaultTimeout = 30) {
    this._session = new Soup.Session();
    this._session.timeout = defaultTimeout;
    this._session.user_agent = 'ALI';
  }
  /*******************************************
   *              HTTP Methods               *
   *******************************************/
  ...
  /**
   * Performs an HTTP Get request to determind network connectivuty
   */
  public async hasInternet(): Promise<boolean> {
    try {
      const response = await this.get(this._networkCheck, {
        responseType: "text",
        timeout: 1000, // remember this is converted to 1 second
      });
      // Accept any 2xx or 3xx as "connected"
